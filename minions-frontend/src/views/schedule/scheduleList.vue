    ...
    <el-dialog title="创建新任务" :visible.sync="dialogVisible">
      <el-form label-width="80px">
        ...
        <el-form-item label="数据库名" v-if="jobType === 'MySQL数据监控任务'">
          <el-select v-model="databaseName" @change="getAllTableByDatabase">
            <el-option v-for="d in databaseNames" :lable="d.databaseName" :key="d.id" :value="d.databaseName"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="表名" v-if="jobType === 'MySQL数据监控任务'">
          <el-select v-model="tableName" @change="getAllFieldNameByTableName">
            <el-option v-for="tn in tableNames" :lable="tn" :key="tn" :value="tn"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="字段名" v-if="jobType === 'MySQL数据监控任务'">
          <el-select v-model="fieldName">
            <el-option v-for="fn in fieldNames" :lable="fn" :key="fn" :value="fn"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getAllDatabases: function(){
      api.getAllDatabases().then(resp => {
        this.databaseNames = resp.data
      })
    },
    getAllTableByDatabase() {
      api.getAllTableByDatabase(this.databaseName).then(resp => {
        this.tableNames = resp.data
      })
    },
    getAllFieldNameByTableName(){
      api.getAllFieldNameByTableName(this.databaseName, this.tableName).then(resp => {
        this.fieldNames = resp.data
      })
    }
  }
}
